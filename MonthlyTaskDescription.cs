...
namespace BulletJournal
{
    public partial class MonthlyTaskDescription : Form
    {
        ...
        public MonthlyTaskDescription(JournalTask.EntryMode _entryMode, int _id, int _layer, JournalTask.EntryType _entryType = JournalTask.EntryType.none)
        {
            InitializeComponent();
            ...
            if (mode == JournalTask.EntryMode.migrate)
            {
                ...
                switch (entryType)
                {
                    case JournalTask.EntryType.daily:
                        MigrationHelper.MigrateDailyToMonthly(id, insertedId);
                        break;
                    case JournalTask.EntryType.monthly:
                        MigrationHelper.MigrateMonthlyToMonthly(id, insertedId);
                        break;
                    case JournalTask.EntryType.future:
                        MigrationHelper.MigrateFutureToMonthly(id, insertedId);
                        break;
                    default:
                        break;
                }
